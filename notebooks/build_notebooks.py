#!/usr/bin/env python3
"""Generate two clean SFT Colab notebooks. Run once to produce:

  - sft_qwen2_5_vl.ipynb   — Unsloth + LoRA on Qwen2.5-VL-7B-Instruct-bnb-4bit
  - sft_kimi_vl.ipynb      — transformers 4.48.2 + PEFT 4-bit + TRL on Kimi-VL-A3B-Thinking-2506

Design constraints (validated against current Unsloth + Kimi-VL docs, Nov 2026):
- Pre-quantized models where available (saves Colab download + memory).
- Pinned dep versions known to be ABI-compatible (numpy>=2.0 + transformers
  the model card recommends).
- Cell 0.5 fails fast on broken imports.
- Cell 6.5 runs ONE training step before committing GPU hours.
- All code cells pass ast.parse(); data-prep tested locally against the
  real COLA Cloud dataset (1714 valid (image, JSON) pairs).
"""
...
# ────────────────────────────────────────────────────────────────────────────
# Cell helpers — each returns a (kind, lines) tuple.
# ────────────────────────────────────────────────────────────────────────────

def md(*lines): return ("markdown", list(lines))
def code(*lines): return ("code", list(lines))


# ────────────────────────────────────────────────────────────────────────────
# Shared cell content (identical across both notebooks).
# ────────────────────────────────────────────────────────────────────────────

def header_cell(model_name, base_model, library, runtime_advice):
    return md(
        f"# Fine-tune **{model_name}** for TTB Label Verification\n",
        "\n",
        f"Self-contained Colab notebook. LoRA fine-tune on `{base_model}` using {library}. "
        "Trains on data pulled from the COLA Cloud free sample (CC0, ~1700 image-JSON pairs).\n",
        "\n",
        ...
        "\n",
        f"## Runtime recommendation\n{runtime_advice}\n",
        "\n",
        "## Before you Run All\n",
        "**If you ran a previous SFT session on this VM**, do `Runtime → Disconnect and "
        "delete runtime` first — that clears any cached numpy/transformers state from "
        "a prior install. Then `Runtime → Run all`. The notebook is fully automated; "
        "caffeinate your Mac (`caffeinate -i &`) and walk away.\n",
    )
...
def validate_imports_cell(extra_modules=()):
    extra = ', ' + ', '.join(repr(m) for m in extra_modules) if extra_modules else ''
    return md(
        "## 0.5. Validate imports (fails fast on ABI mismatch)\n",
        "\n",
        "If this cell raises `numpy.dtype size changed` or a similar binary-incompatibility "
...
def drive_cell():
    return md("## 1. Mount Drive + set up work directories\n"), code(
...
def download_data_cell():
    return md(
        "## 2. Download COLA Cloud free sample (CC0)\n",
        "\n",
        "1,000 records + ~1,750 image rows. ~500 KB zip. Sourced from the official TTB "
        "Public COLA Registry, packaged for direct fetch.\n",
    ), code(
...
def build_pairs_cell():
    """Tested locally — produces 1714 valid JSON pairs from the real COLA Cloud data."""
    return md(
        "## 3. Build (image, structured-JSON) training pairs\n",
        "\n",
        "For each label image with usable metadata, synthesize the structured JSON the "
        "model should learn. The schema matches the backend's `ExtractedLabel` shape so "
        "the SFT'd adapter is drop-in for `INFERENCE_MODE=sft`.\n",
        "\n",
...
def download_images_cell():
    return md("## 4. Download label images (parallel, ~3 min)\n"), code(
...
def split_cell():
    return md("## 5. Train / val / test split (80 / 10 / 10)\n"), code(
        "import random, json\n",
...
def model_load_qwen():
    return md(
        "## 6. Load Qwen2.5-VL-7B (pre-quantized 4-bit) + apply LoRA\n",
        "\n",
        "Uses Unsloth's pre-quantized `unsloth/Qwen2.5-VL-7B-Instruct-bnb-4bit` — same "
        "weights as the upstream model but ships in 4-bit so the Colab download is faster.\n",
    ), code(
        "import torch\n",
        ...
        "    model,\n",
        "    finetune_vision_layers     = False,  # vision tower frozen (saves memory; fine for OCR-like task)\n",
        ...
        "FastVisionModel.for_training(model)  # explicit training mode\n",
    )
...
def model_load_kimi():
    return md(
        ...
        "\n",
        "Kimi-VL has a custom MoE architecture (16B total / 3B active per token). We load "
        "in 4-bit via bitsandbytes (full bf16 wouldn't fit on a Colab A100) and apply LoRA "
        "via PEFT targeting all linear projections.\n",
    ), code(
        "import torch\n",
...
def format_dataset_qwen():
    return md(
        "## 7. Format dataset for SFT (Unsloth conversation schema)\n",
    ), code(
...
def smoke_test_cell():
    """Single training step on 4 samples — surfaces any bug in ~30 seconds."""
    return md(
        "## 7.5. Smoke test — 1 training step on 4 samples\n",
        "\n",
        "**This cell runs in ~30 seconds and catches almost every bug** (broken imports, "
        "OOM, model-config mismatches, data-collator issues) BEFORE you commit to the "
        "full ~2-hour training run. If it crashes, fix the issue HERE — don't proceed.\n",
    ), code(
...
def trainer_qwen():
    return md("## 8. Full training run (2 epochs)\n"), code(
        "from trl import SFTTrainer, SFTConfig\n",
        "from unsloth.trainer import UnslothVisionDataCollator\n",
        "\n",
        "trainer = SFTTrainer(\n",
...
def eval_qwen():
    return md(
        ...
        "    inputs = tokenizer(text, images=img, return_tensors='pt').to(model.device)\n",
        "    with torch.no_grad():\n",
        "        out = model.generate(**inputs, max_new_tokens=512, do_sample=False)\n",
        "    txt = tokenizer.batch_decode(out[:, inputs['input_ids'].shape[1]:], skip_special_tokens=True)[0]\n",
        "    try:\n",
        ...
        "_run_eval(test, _gen)  # defined in next cell\n",
    )
...
def eval_shared_cell():
    """Per-field scoring loop — same for both models."""
    return md("## 9.5. Per-field scoring helper\n"), code(
...
INTEGRATION_NOTE = md(
    ...
    "```\n",
    "\n",
    "We'll add `backend/app/extractors/sft.py` (loads base model + LoRA via vLLM or "
    "transformers) and register `INFERENCE_MODE=sft` in the factory. Then\n",
...
def main():
    ...
    qwen_cells = [
        header_cell(
            "Qwen2.5-VL-7B", "unsloth/Qwen2.5-VL-7B-Instruct-bnb-4bit", "Unsloth",
            "**A100 (Colab Pro)** is recommended — full 2-epoch run is ~3 hours. "
            "T4 will OOM on the 7B model; if you only have T4 access, use the Kimi notebook instead."
        ),
        *install_qwen(),
        model_tag_cell("qwen2_5_vl_7b", "unsloth/Qwen2.5-VL-7B-Instruct-bnb-4bit"),
        *validate_imports_cell(extra_modules=('unsloth',)),
        *drive_cell(),
        *download_data_cell(),
        *build_pairs_cell(),
        *download_images_cell(),
        *split_cell(),
        SYSTEM_PROMPT_CELL,
        *model_load_qwen(),
        *format_dataset_qwen(),
    ...
    write(OUT / 'sft_qwen2_5_vl.ipynb', build_notebook(qwen_cells))
    print(f'wrote {OUT / "sft_qwen2_5_vl.ipynb"} ({len(qwen_cells)} cells)')

    kimi_cells = [
        header_cell(
            "Kimi-VL-A3B-Thinking", "moonshotai/Kimi-VL-A3B-Thinking-2506",
            "transformers 4.48.2 + PEFT + TRL",
            "**A100 ideal** (~2 hr); **T4 works** but ~5-6 hr because the model is "
            "16B-MoE / 3B-active and 4-bit quantization fits in 16 GB VRAM."
        ),
        *install_kimi(),
        model_tag_cell("kimi_vl_a3b_thinking", "moonshotai/Kimi-VL-A3B-Thinking-2506"),
        *validate_imports_cell(),
        *drive_cell(),
        *download_data_cell(),
        *build_pairs_cell(),
        *download_images_cell(),
    ...
    write(OUT / 'sft_kimi_vl.ipynb', build_notebook(kimi_cells))
    print(f'wrote {OUT / "sft_kimi_vl.ipynb"} ({len(kimi_cells)} cells)')
