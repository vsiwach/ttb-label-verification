...
VALIDATE_IMPORTS = md(
    "## 0.5. Validate imports (fails fast on ABI / version mismatch)\n",
    "\n",
    "If this cell errors with `numpy.dtype size changed` or a missing module, **stop**\n",
    "— do not run the rest of the notebook until imports are clean. Most likely fix:\n",
    "**Runtime → Disconnect and delete runtime** (fresh VM), then Run all again.\n",
), code(
    "# Fail fast on import errors so we catch ABI issues before training.\n",
    "import importlib, sys\n",
    "_required = ['torch', 'transformers', 'accelerate', 'peft', 'trl',\n",
    "             'bitsandbytes', 'datasets', 'PIL', 'tqdm']\n",
    "_missing = []\n",
    ...
    "        _missing.append((mod, e))\n",
    "if _missing:\n",
    "    raise RuntimeError(\n",
    "        f'{len(_missing)} import(s) failed. Most common cause: numpy ABI mismatch — '\n",
    "        f'do Runtime → Disconnect and delete runtime, then Run all in a clean VM.'\n",
    "    )\n",
    "print('\\n✅ All imports clean.')\n",
)
...
def format_dataset_kimi():
    return md(
        "## 6. Format dataset + collator for TRL SFTTrainer\n",
    ), code(
        "from PIL import Image\n",
        ...
        "print('train examples:', len(train_ds), 'val examples:', len(val_ds))\n",
        "\n",
        "# Define the collator HERE so the smoke-test cell below can use it.\n",
        "def _collate(batch):\n",
        "    texts, images = [], []\n",
        "    for ex in batch:\n",
        "        msgs = ex['messages']\n",
...
SMOKE_TEST = md(
    "## 6.5. Smoke test — single training step on 4 samples\n",
    "\n",
    "**Runs in ~30 seconds and surfaces any broken imports / data plumbing /\n",
    "OOM / model-config issues BEFORE you commit to the 3-hour training run.**\n",
    "If this cell crashes, fix the issue here — don't proceed to full training.\n",
), code(
    "# Tiny subset + 1 step to validate the entire pipeline.\n",
    "if hasattr(train_ds, 'select'):  # HuggingFace Dataset (Kimi path)\n",
    "    _smoke_ds = train_ds.select(range(min(4, len(train_ds))))\n",
    "else:                              # plain list of dicts (Qwen path)\n",
    "    _smoke_ds = list(train_ds)[:4]\n",
    "print(f'Smoke-test on {len(_smoke_ds)} examples...')\n",
    "\n",
    "try:\n",
    "    from trl import SFTTrainer, SFTConfig\n",
    "    try:\n",
    "        from unsloth import is_bf16_supported\n",
    "        _bf16 = is_bf16_supported()\n",
    "    except ImportError:\n",
    "        import torch; _bf16 = torch.cuda.is_bf16_supported() if torch.cuda.is_available() else False\n",
    "\n",
    ...
    "        max_seq_length = 2048,\n",
    "    )\n",
    "    # Pass collator only when present (Unsloth path defines one)\n",
    "    _kw = {'args': _smoke_args, 'train_dataset': _smoke_ds, 'model': model}\n",
    "    if 'UnslothVisionDataCollator' in globals():\n",
    "        _kw['data_collator'] = UnslothVisionDataCollator(model, tokenizer)\n",
    "        _kw['tokenizer'] = tokenizer\n",
    "    elif '_collate' in globals():\n",
    "        _kw['data_collator'] = _collate\n",
    "    _smoke = SFTTrainer(**_kw)\n",
    "    _smoke.train()\n",
    "    print('\\n✅ SMOKE TEST PASSED. Proceeding to full training is safe.')\n",
    "except Exception as e:\n",
    "    print('\\n❌ SMOKE TEST FAILED:')\n",
    "    import traceback; traceback.print_exc()\n",
    "    print('\\nFix the above before running the full trainer cell. Do not commit GPU hours to a broken pipeline.')\n",
    "    raise\n",
)
...
def trainer_kimi():
    return md(
        "## 7. Train — TRL SFTTrainer (collator was defined in cell 6)\n",
    ), code(
        "from trl import SFTTrainer, SFTConfig\n",
        "\n",
        "trainer = SFTTrainer(\n",
...
def main():
    ...
    qwen_cells = [
        ...
        md("## 0. Install dependencies & set model tag\n",
           "\n",
           "Installs numpy 2.x + recent transformers / bitsandbytes / Unsloth — the\n",
           "stack of versions known to be ABI-compatible on Colab today. If you previously\n",
           "ran a session that pinned `numpy<2.0`, do **Runtime → Disconnect and delete runtime**\n",
           "first (not just restart) to get a clean VM, then Run all.\n"),
        code("!pip install -q --upgrade pip\n",
             "# numpy 2.x is what modern transformers/bitsandbytes wheels are compiled against;\n",
             "# pinning numpy<2 causes ABI mismatches against the C extensions.\n",
             "!pip install -q --upgrade 'numpy>=2.0'\n",
             "!pip install -q --upgrade \"unsloth[colab-new]\" 'trl<0.10.0' peft accelerate 'bitsandbytes>=0.44'\n",
    ...
    kimi_cells = [
        ...
        md("## 0. Install dependencies & set model tag\n",
           "\n",
           "Installs numpy 2.x + known-compatible transformers / bitsandbytes / peft / trl. \n",
           "If you previously ran a session that pinned `numpy<2.0`, do **Runtime →\n",
           "Disconnect and delete runtime** first to get a clean VM.\n"),
        code("!pip install -q --upgrade pip\n",
             "# numpy 2.x is what modern wheels are built against; pinning <2 causes ABI mismatch.\n",
             "!pip install -q --upgrade 'numpy>=2.0'\n",
             "# Kimi-VL-A3B-Thinking requires transformers >= 4.48 + trust_remote_code\n",
             "!pip install -q --upgrade 'transformers>=4.48' accelerate 'bitsandbytes>=0.44' peft 'trl<0.10.0'\n",
             "!pip install -q --upgrade datasets pillow requests tqdm sentencepiece\n",
             "\n",
             "import numpy, transformers; print('numpy:', numpy.__version__, '| transformers:', transformers.__version__)\n"),
