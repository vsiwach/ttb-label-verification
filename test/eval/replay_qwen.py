#!/usr/bin/env python3
"""End-to-end Qwen vs Claude scoring — replay edition.
...
outputs from a JSONL fixture (produced by the Colab eval cell at
notebooks/eval_qwen_on_claude_testset.py).
...
def _print_side_by_side(qwen: dict, claude_path: Path) -> None:
    ...
    print()
    print("=" * 80)
    print(f"{'Metric':<32} {'Claude':>12} {'Qwen':>12}   {'Δ (Qwen − Claude)':>20}")
    print("-" * 80)
...
def main() -> int:
    ...
    ap.add_argument("--jsonl", default=str(Path.home() / "Downloads" / "qwen_outputs.jsonl"),
                    help="Qwen outputs JSONL produced by the Colab eval cell")
    ap.add_argument("--csv",   default=str(REPO_ROOT / "test" / "eval" / "data" / "cola_sample.csv"),
                    help="Ground-truth CSV (same one run_eval.py uses)")
    ap.add_argument("--out",   default=str(REPO_ROOT / "test" / "eval" / "qwen_report.json"),
                    help="Write the Qwen report here (same shape as test/eval/report.json)")
